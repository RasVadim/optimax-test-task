...
export const getCurrentProduct = (state: RootState) => state.products.currentProductId
...
export const checkInCartProduct = (id: number) =>
    (state: RootState) =>
        state.cart.cart.find((product) => product.id === id)
export const getCounterValueById = (counterId: string) =>
    (state: RootState) => +state.form.CounterForm?.values?.[counterId]